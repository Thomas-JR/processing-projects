// background image source http://www.illustratedmaps.com/IllustratedMaps/Z-Town-by-blocks_Map_illustration.html
// Zombie img source http://plantsvszombies.wikia.com/wiki/File:Regular_Zombie.png
// Peashooter img source http://vsbattles.wikia.com/wiki/Peashooter_(Plants_vs_Zombies)
...
void setup() {
  ...
  state.zombie = loadImage("Zombie.png");
  state.plant = loadImage("Pea_Shooter.png");
  ...
  for (int p = 0; p < people.length; p++) {
    ...
    for (int place = 0; place < chances.size(); place++) {
      arbitrary += chances.get(place);
      if (location <= arbitrary) {
...
void draw() {
  //Background
  image(state.back, width/2, height/2);
  fill(255, 150);
  rect(0, 0, width, height);
  //Possibility to drag nodes
  if (mousePressed && state.play) {
    if (state.selected >= 0) {
