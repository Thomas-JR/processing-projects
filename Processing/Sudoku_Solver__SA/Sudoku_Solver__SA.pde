...
int[][] totalDistance(int[][] array) {
  int counter, num_zeroes;
  // first array of 9 is the count wrong in the rows
  // second array of 9 is the count wrong in the columns
  int[][] doublesCount = new int[2][9];
  for (int j = 0; j < 9; j++) {
    // Find the num incorrect in rows
    counter = 0;
    for (int i = 0; i < 9; i++) {
        counter |= (int)Math.pow(2, array[j][i]-1);
    }
    num_zeroes = 0;
    ...
    doublesCount[0][j] = num_zeroes;
    
  // Find the num incorrect in columns
    counter = 0;
    for (int i = 0; i < 9; i++) {
        counter |= (int)Math.pow(2, array[i][j]-1);
    }
    num_zeroes = 0;
    for(int i = 0; i < 9; i++, counter >>= 1) {
      if ((counter & 1) == 0)
