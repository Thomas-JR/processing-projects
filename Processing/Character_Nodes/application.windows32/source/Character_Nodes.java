...
public class Character_Nodes extends PApplet {
...
public void setup() {
  ...
  for (int i = 0; i < chars; i++) {
    ...
    int p = PApplet.parseInt(random(People.size()));
    //Iterate through the amount of people
  ...
  if (mousePressed &! lastPressed) {
    ...
    for (int c = 0; c < characters.length; c++) {
      if (dist(mouseX, mouseY, characters[c].pos.x, characters[c].pos.y) < characters[c].size/2) {
        ...
        characters[c].size = characters[c].radius*1.2f;
        canChange = false;
      }
...
class Char {
  ...
  public void update() {
    ...
    pos.x = lerp(pos.x, targetPos.x, 0.05f);
    pos.y = lerp(pos.y, targetPos.y, 0.05f);

    //Show the node
    if (dist(mouseX, mouseY, pos.x, pos.y) < radius / 2)
      targetSize = radius * 1.4f;
    else
      targetSize = radius;
    size = lerp(size, targetSize, 0.2f);
    strokeWeight(2);
    if (background < 255/2)
      stroke(255);
    else
...
public void updateBackground() {
  ...
  fill(255);
  text("BACKGROUND", 255/2, 37.5f);
  text("i", 40, height-40+30/3);
  rect(background, 25, 20, 55);
}

public void relationshipTab() {
  fill(255-map(background, 0, 255, 100, 155));
...
public void crs() {
  ...
  if (mousePressed)
    ...
    tDistance = 10;
  distance = lerp(distance, tDistance, 0.3f);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Character_Nodes" };
    if (passedArgs != null) {
